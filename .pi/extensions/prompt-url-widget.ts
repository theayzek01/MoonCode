import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "@moodcli/coding-agent";
...
async function fetchGhMetadata(
	...
	try {
		const result = await moodcli.exec("gh", args);
...
export default function promptUrlWidgetExtension(moodcli: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, title?: string) => {
		...
		const currentName = moodcli.getSessionName()?.trim();
		if (!currentName) {
			moodcli.setSessionName(desiredName);
			return;
		}
	...
	moodcli.on("before_agent_start", async (event, ctx) => {
		...
		void fetchGhMetadata(moodcli, match.kind, match.url).then((meta) => {
	...
	moodcli.on("session_switch", async (_event, ctx) => {
