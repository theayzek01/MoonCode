...
```bash
npm install @moodcli/ai
```

TypeBox exports are re-exported from `@moodcli/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@moodcli/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@moodcli/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@moodcli/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@moodcli/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@moodcli/ai';
...
```typescript
import { getModel, stream } from '@moodcli/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@moodcli/ai';
...
```typescript
import { Model, stream } from '@moodcli/ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are moodcli thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@moodcli/ai';
...
```typescript
import { getModel, complete, Context } from '@moodcli/ai';
...
```typescript
import { Context, getModel, complete } from '@moodcli/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@moodcli/ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@moodcli/ai';
...
```bash
npx @moodcli/ai login              # interactive provider selection
npx @moodcli/ai login anthropic    # login to specific provider
npx @moodcli/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@moodcli/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@moodcli/ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@moodcli/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@moodcli/ai`
