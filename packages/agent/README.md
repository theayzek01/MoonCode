...
Stateful agent with tool execution and event streaming. Built on `@moodcli/ai`.

## Installation

```bash
npm install @moodcli/agent-core
```
...
```typescript
import { Agent, streamProxy } from "@moodcli/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@moodcli/agent-core";
