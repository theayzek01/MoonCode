...
const PLUGIN_DIR = join(homedir(), ".mooncode", "openui", "mooncode-plugin");
const PLUGIN_REPO = "thesysdev/mooncode-os/packages/mooncode-plugin";

const PREFIX = "[mooncode-os]";
...
function readMoonCodeConfig() {
  log("==> Reading MoonCode config...");

  const configPath = join(homedir(), ".mooncode", "mooncode.json");
  if (!existsSync(configPath)) {
    fatal("MoonCode config not found. Is MoonCode installed?");
  }
  ...
  if (!token) {
    fatal("Gateway auth token not set. Run: mooncode auth token");
  }
...
function downloadPlugin() {
  log("==> Downloading MoonCode OS plugin...");
  log(`    From: ${PLUGIN_REPO}`);

  mkdirSync(join(homedir(), ".mooncode", "openui"), { recursive: true });
  ...
  const pluginManifest = join(PLUGIN_DIR, "mooncode.plugin.json");
  if (!existsSync(pluginManifest)) {
    fatal(
      `Plugin download produced an invalid directory at ${PLUGIN_DIR} (missing mooncode.plugin.json). ` +
...
function installPlugin() {
  log("==> Installing MoonCode OS plugin...");
  if (isPluginRegistered()) {
    log("    Plugin already registered, reinstalling to pick up updates...");
    try {
      execSync("mooncode plugins uninstall mooncode-os-plugin", {
        stdio: "inherit",
      });
  ...
  execSync(`mooncode plugins install -l "${PLUGIN_DIR}"`, {
    stdio: "inherit",
  });
  log("    Plugin installed.");
}

const OPENCLAW_CONFIG_PATH = join(homedir(), ".mooncode", "mooncode.json");
...
function isPluginRegistered() {
  try {
    const output = execSync("mooncode plugins list", { encoding: "utf8" });
    return output.includes("mooncode-os-plugin");
  } catch {
    return false;
  }
}

// TODO: mooncode onboard sets tools.profile="coding" by default, which blocks all plugin tools
// via applyToolPolicyPipeline unless tools.alsoAllow includes "group:plugins" or the plugin id.
// Ideally mooncode's onboarding wizard (or `mooncode plugins install`) should detect a restrictive
// profile and prompt the user to add alsoAllow, similar to how addAllowedOrigin works here.
// Tracked: remove this workaround once mooncode handles it natively.
function ensurePluginToolsAllowed() {
  log("==> Ensuring plugin tools are accessible...");

  if (!existsSync(OPENCLAW_CONFIG_PATH)) {
    log("    WARNING: MoonCode config not found, skipping.");
    return;
  }
...
function restartGateway() {
  log("==> Restarting MoonCode gateway...");
  try {
    execSync("mooncode gateway restart", { stdio: "inherit" });
    log("    Gateway restarted.");
  } catch {
    log(
      "    WARNING: Could not restart gateway automatically. Please run: mooncode restart",
    );
  }
}
...
function saveConfig({ tunnelId, gatewayUrl, apiBase, tunnelToken }) {
  ...
  const dir = join(homedir(), ".mooncode", "openui");
...
const CONFIG_PATH = join(homedir(), ".mooncode", "openui", "config.json");
...
function removePlugin() {
  log("==> Removing MoonCode OS plugin...");

  try {
    execSync("mooncode plugins uninstall mooncode-os-plugin", { stdio: "inherit" });
...
async function install(args) {
  if (!commandExists("mooncode")) {
    fatal(
      "MoonCode CLI not found. Install it first: https://mooncode.com/docs/install",
    );
  }

  const { port, token } = readMoonCodeConfig();
