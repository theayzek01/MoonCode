...
import { jsonResult } from "mooncode/plugin-sdk/agent-runtime";
import type {
  GatewayRequestHandlerOptions,
  MoonCodePluginToolContext,
} from "mooncode/plugin-sdk/core";
import { definePluginEntry, emptyPluginConfigSchema } from "mooncode/plugin-sdk/plugin-entry";
...
export default definePluginEntry({
  id: "mooncode-os-plugin",
  name: "Claw — OpenUI for MoonCode",
  description:
  ...
  register(api) {
    api.logger.info("[mooncode-os-plugin] register() called — plugin loaded OK");

    // ── Static UI route ─────────────────────────────────────────────────────
    // Serves the mooncode-client static export bundled into ../static/ at install
    ...
    const ROUTE_PREFIX = "/plugins/mooncodeos";
    ...
    const serveFile = (res: ServerResponse, absPath: string): void => {
      ...
      createReadStream(absPath)
        .on("error", (err) => {
          api.logger.warn(`[mooncode-os-plugin] static stream error ${absPath}: ${err}`);
          if (!res.headersSent) {
            res.writeHead(500);
          }
          res.end();
    ...
    api.registerHttpRoute({
      ...
      handler: async (req: IncomingMessage, res: ServerResponse) => {
        ...
        // Strip the route prefix so /plugins/mooncodeos/_next/x → /_next/x
    ...
    const uiHost =
      api.config.gateway?.bind === "custom" && api.config.gateway?.customBindHost
        ...
        : "127.0.0.1";
    api.logger.info(
      `[mooncode-os-plugin] workspace UI mounted at http://${uiHost}:${uiPort}${ROUTE_PREFIX}/ (root ${STATIC_ROOT})`,
    );
    ...
    api.registerCli(
      ({ program, config }) => {
        const group = program
          .command("os")
          .description("MoonCode OS — Generative UI client controls");

        group
          .command("url")
          .description("Print the MoonCode OS setup URL with auth (gateway+token in fragment)")
          .action(() => {
            ...
            if (bind === "custom" && customHost) {
              ...
              process.stderr.write(
                "[mooncode-os] gateway.bind=tailnet detected — using 127.0.0.1 in the URL. " +
            ...
            if (typeof tokenInput !== "string" || !tokenInput) {
              ...
              throw new Error(`${reason}. Run \`mooncode onboard\` to set one.`);
            }
    ...
    api.on("before_prompt_build", (_event, ctx) => {
      if (!ctx.sessionKey?.endsWith(":mooncode-os")) {
        return;
      }
    ...
    const markAppSkillRead = (sessionKey: string): void => {
      ...
      try {
        ...
        api.logger.warn(`[mooncode-os-plugin] failed to persist app-skill gate: ${err}`);
      }
    };

    api.on("before_tool_call", (event, ctx) => {
      const sessionKey = ctx.sessionKey;
      if (typeof sessionKey !== "string" || !sessionKey.endsWith(":mooncode-os")) {
        return;
      }
      ...
      if (
        ...
        api.logger.info(
          `[mooncode-os-plugin] ${event.toolName} blocked — openui-app skill not read in session ${sessionKey}`,
        );
    ...
    const getStore = (): ArtifactStore => {
      if (!store) {
        ...
        api.logger.info(`[mooncode-os-plugin] initialising ArtifactStore at: ${stateDir}`);
    ...
    // ── Upload store — durable attachment bytes (MoonCode's media dir TTLs) ──
    ...
    const resolveDatabasePath = async (namespace: string): Promise<string> => {
      const stateDir = api.runtime.state.resolveStateDir();
      const dbDir = path.join(stateDir, "plugins", "mooncode-os", "db");
    ...
    api.logger.info("[mooncode-os-plugin] registering tools…");

    api.registerTool(
      (ctx: MoonCodePluginToolContext) => ({
    ...
    api.registerTool(
      (_ctx: MoonCodePluginToolContext) => ({
    ...
    api.registerTool(
      (ctx: MoonCodePluginToolContext) => ({
        ...
        execute: async (_callId: string, params: { title: string; code: string }) => {
          api.logger.info(
            `[mooncode-os-plugin] app_create: title="${params.title}" code=${params.code.length} chars`,
          );
          const lint = lintOpenUICode(params.code);
          if (!lint.ok) {
            api.logger.info(
              `[mooncode-os-plugin] app_create lint: ${lint.findings.length} finding(s) — ${lint.summary.slice(0, 180)}`,
            );
          }
          ...
          api.logger.info(`[mooncode-os-plugin] app_create → saved app ${app.id}`);
          return jsonResult({
            id: app.id,
            title: app.title,
    ...
    api.registerTool(
      () => ({
        ...
        execute: async (_callId: string, params: { id: string; patch: string }) => {
          ...
          api.logger.info(
            `[mooncode-os-plugin] app_update: id=${params.id} patch=${params.patch.length} chars`,
          );
          ...
          if (!lint.ok) {
            api.logger.info(
              `[mooncode-os-plugin] app_update lint: ${lint.findings.length} finding(s) — ${lint.summary.slice(0, 180)}`,
            );
          }

          const updated = await getAppStore().update(params.id, { content: merged });
          api.logger.info(`[mooncode-os-plugin] app_update → updated app ${updated.id}`);
          return jsonResult({
            id: updated.id,
    ...
    api.logger.info("[mooncode-os-plugin] all tools registered");
    ...
    //
    // Namespaced under `mooncodeos.*` because mooncode 2026.5.x ships a built-in
    ...
    // name. See: mooncode/docs/gateway/protocol.md ("artifacts.list…").
    api.registerGatewayMethod(
      "mooncodeos.artifacts.list",
    ...
    api.registerGatewayMethod(
      "mooncodeos.uploads.deleteBySession",
      async ({ params, respond }: GatewayRequestHandlerOptions) => {
        try {
          const sessionKey = typeof params["sessionKey"] === "string" ? params["sessionKey"] : "";
          if (!sessionKey) {
    ...
    api.registerGatewayMethod(
      "mooncodeos.notifications.upsert",
      async ({ params, respond }: GatewayRequestHandlerOptions) => {
        try {
    ...
    const invokeExecTool = async (args: Record<string, unknown>): Promise<unknown> => {
      ...
      api.logger.info(`[mooncode-os-plugin] invokeTool(exec): command=${command.slice(0, 120)}`);
      try {
    ...
    const invokeReadTool = async (args: Record<string, unknown>): Promise<unknown> => {
      ...
      api.logger.info(`[mooncode-os-plugin] invokeTool(read): path=${filePath}`);
    ...
    api.registerGatewayMethod(
      ...
      async ({ params, respond }: GatewayRequestHandlerOptions) => {
        ...
        try {
          api.logger.info(`[mooncode-os-plugin] tools.invoke: tool=${toolName}`);
          const result = await invokeTool(toolName, toolArgs, sessionKey);
          respond(true, { result });
        } catch (e) {
          api.logger.error(
            `[mooncode-os-plugin] tools.invoke failed: tool=${toolName} error=${e instanceof Error ? e.message : String(e)}`,
          );
          respond(false, undefined, {
    ...
    api.logger.info("[mooncode-os-plugin] gateway RPC methods registered");
  },
});
