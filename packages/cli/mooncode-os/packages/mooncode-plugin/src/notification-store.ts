...
import { generateSecureUuid } from "mooncode/plugin-sdk/infra-runtime";
...
export class NotificationStore {
  ...
  constructor(stateDir: string) {
    this.dir = path.join(stateDir, "plugins", "mooncode-os", "notifications");
  ...
  private async readAll(): Promise<StoredNotification[]> {
    ...
    try {
      ...
      if (!Array.isArray(parsed)) {
        console.warn(
          `[mooncode-plugin] notifications file at ${this.filePath} is not an array — treating as empty`,
        );
        return [];
      }
      ...
      if (code !== "ENOENT") {
        console.warn(`[mooncode-plugin] failed to read notifications from ${this.filePath}:`, err);
      }
      return [];
    }
  }
