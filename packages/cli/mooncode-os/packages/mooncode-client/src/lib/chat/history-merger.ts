/**
 * Merges MoonCode chat.history messages into UI-ready turns.
 *
 * A single assistant "turn" in MoonCode history spans multiple messages:
...
export function mergeHistoryMessages(raw: ChatHistoryMessage[]): MergedMessage[] {
  ...
  for (const m of raw) {
    if (m.role === "user") {
      flush();
      output.push({
        id: m.__mooncode?.id ?? m.id ?? crypto.randomUUID(),
        role: "user",
    ...
    if (m.role === "assistant") {
      ...
      if (isGatewayInjected) {
        ...
        // as a `gateway-injected` message tagged `mooncodeAbort` whenever a
        ...
        const isAbortSnapshot = Boolean(
          (m as unknown as { mooncodeAbort?: { aborted?: boolean } }).mooncodeAbort?.aborted,
        );
        if (isAbortSnapshot) {
          continue;
        }
    ...
    // System messages with `__mooncode.kind === "compaction"` are markers
    ...
    if (m.role === "system" && m.__mooncode?.kind === "compaction") {
      flush();
      output.push({
        id: m.__mooncode?.id ?? m.id ?? crypto.randomUUID(),
        role: "assistant",
