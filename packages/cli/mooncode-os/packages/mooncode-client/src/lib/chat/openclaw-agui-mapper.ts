...
/**
 * Maps MoonCode gateway events to AG-UI events that @openuidev/react-headless understands.
 *
 * MoonCode streams (event:agent payload):
...
const DEBUG_STORAGE_KEY = "mooncode-os-debug-events";
...
export function createMoonCodeAGUIMapper(onEvent: (event: Record<string, unknown>) => void): {
