...
import { createMoonCodeAGUIMapper } from "@/lib/chat/mooncode-agui-mapper";
...
const log = (...args: unknown[]) => console.info("[claw:mooncode-engine]", ...args);
const warn = (...args: unknown[]) => console.warn("[claw:mooncode-engine]", ...args);
...
export interface MoonCodeEngineEvents {
...
export class MoonCodeEngine implements Engine {
  readonly id: string;
   ...
   *  `agent:main:main:mooncode-os`, splitting reads across two transcripts. */
  ...
  private events: MoonCodeEngineEvents;
  ...
  constructor(config: MoonCodeEngineConfig, events: MoonCodeEngineEvents) {
    this.id = config.id;
  ...
  readonly artifacts: ArtifactStore = {
    ...
    deleteArtifact: async (artifactId: string): Promise<void> => {
      await this._request("mooncodeos.artifacts.delete", { id: artifactId });
    },
  };
  ...
  readonly apps: AppStore = {
    listApps: async (): Promise<AppSummary[]> => {
      try {
        const result = await this._request<{ apps: AppSummary[] }>("mooncodeos.apps.list", {});
        return result?.apps ?? [];
      } catch (error) {
    ...
    getApp: async (appId: string): Promise<AppRecord | null> => {
      try {
        const result = await this._request<{ app: AppRecord | null }>("mooncodeos.apps.get", {
          id: appId,
        });
    ...
    deleteApp: async (appId: string): Promise<void> => {
      await this._request("mooncodeos.apps.delete", { id: appId });
    },
    ...
    invokeTool: async (
      ...
      const result = await this._request<{ result: unknown }>("mooncodeos.tools.invoke", {
        tool_name: tool,
        tool_args: args,
  ...
  readonly uploads: UploadStore = {
    ...
    deleteUpload: async (id: string): Promise<void> => {
      try {
        await this._request("mooncodeos.uploads.delete", { id });
      } catch (error) {
  ...
  async sendMessage(
    ...
    const mapper = createMoonCodeAGUIMapper(write);
    ...
    // chat.history on the next reload — matching the mooncode web client's
    ...
    try {
      await this.socket?.request("chat.send", {
        ...
        // mooncode web client (ui/src/ui/controllers/chat.ts:410) does the
  ...
  async listNotifications(): Promise<NotificationRecord[]> {
    ...
    try {
      const result = await this._request<NotificationsListResult>("mooncodeos.notifications.list");
      this.notificationMethodState = "supported";
      return result?.notifications ?? [];
    } catch (e) {
      if (this.isUnknownMethodError(e, "mooncodeos.notifications.list")) {
  ...
  async markNotificationsRead(ids?: string[]): Promise<boolean> {
    ...
    try {
      await this._request("mooncodeos.notifications.read", {
        ...(ids && ids.length > 0 ? { ids } : {}),
      });
      this.notificationMethodState = "supported";
      return true;
    } catch (e) {
      if (this.isUnknownMethodError(e, "mooncodeos.notifications.read")) {
        this.notificationMethodState = "unsupported";
  ...
  async upsertNotification(
    ...
    try {
      await this._request("mooncodeos.notifications.upsert", notification);
      this.notificationMethodState = "supported";
      return true;
    } catch (e) {
      if (this.isUnknownMethodError(e, "mooncodeos.notifications.upsert")) {
        this.notificationMethodState = "unsupported";
   ...
   * carry `model.primary` per agent in this mooncode build — config is the
  ...
  private async _refreshConfig(): Promise<void> {
    try {
      ...
      // Mirror mooncode's `resolveDefaultAgentId`: first configured agent,
  ...
  private async _deleteSessionRecord(sessionId: string): Promise<void> {
    ...
    try {
      await this._request("mooncodeos.uploads.deleteBySession", { sessionKey });
    } catch (error) {
  ...
  private async _loadHistory(sessionId: string): Promise<StoredMessage[]> {
    ...
    // post-hydration encoded form `agent:main:main:mooncode-os`.
