...
import type { CompactSessionResult } from "@/lib/engines/mooncode/MoonCodeEngine";
import type {
  AppRecord,
  AppStore,
  AppSummary,
...
export function ThreadArea({
   ...
   *
   * Resolution order:
   *  1. Parse `agent:<id>:<slot>:mooncode-os` directly from the route key
  ...
  const addFiles = useCallback(
    async (files: File[]) => {
      ...
      // after MoonCode's 2-minute media TTL expires. Use the resolved session
  ...
  return (
    <UploadsProvider store={uploads} sessionKey={sessionKey} seeds={uploadSeeds}>
      <div className="flex h-full min-w-0 flex-1 overflow-hidden bg-background dark:bg-sunk">
        ...
        <ClawThreadContainer className="mooncode-os-thread-container min-w-0 flex-1">
          <ThreadHeader
          ...
          {(() => {
            const composerEl = (
              <SessionComposer
                ...
                {...(() => {
                  // Map mooncode's SessionRow fields onto the ring's
