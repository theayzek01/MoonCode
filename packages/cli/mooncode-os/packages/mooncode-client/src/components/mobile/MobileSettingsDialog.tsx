...
export function MobileSettingsDialog({
  ...
  useEffect(() => {
    ...
    if (connectionState === ConnectionState.CONNECTED) {
      ...
      submitSnapshotRef.current = null;
      hasLeftSnapshotRef.current = false;
      setError("Gateway rejected the auth token. Run `mooncode onboard` to set a fresh one.");
    }
      ...
      >
        <form onSubmit={handleSubmit} className="mt-ml flex flex-col gap-ml">
          <p className="text-sm text-text-neutral-tertiary">
            Connect Claw to your MoonCode gateway. The fastest way is to run{" "}
            <code className="rounded bg-foreground px-1 font-mono">mooncode os url</code> in a
            terminal — it opens this page pre-authenticated. To paste manually, open{" "}
            <code className="rounded bg-foreground px-1 font-mono">~/.mooncode/mooncode.json</code>{" "}
          ...
          <div className="flex flex-col gap-xs">
            ...
            <p className="text-sm text-text-neutral-tertiary">
              ...
              <code className="rounded bg-foreground px-1 font-mono">
                ~/.mooncode/mooncode.json
              </code>{" "}
              (<code className="font-mono">gateway.auth.token</code>) or run{" "}
              <code className="rounded bg-foreground px-1 font-mono">mooncode onboard</code> to set
