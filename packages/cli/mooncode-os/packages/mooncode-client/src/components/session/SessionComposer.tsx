 ...
 * client-side) and MoonCode's gateway commands (which get intercepted by the
...
export function SessionComposer({
  ...
  useEffect(() => {
    ...
    window.addEventListener("mooncode-os:prime-composer", listener as EventListener);
    return () =>
      window.removeEventListener("mooncode-os:prime-composer", listener as EventListener);
  }, []);
  ...
  return (
    <div
      className={`mooncode-os-session-composer relative mb-1 w-full rounded-xl border border-border-default/25 bg-foreground p-[2px] sm:mb-3 dark:border-border-default/8 ${
          ...
          {onModelChange
            ? (() => {
                ...
                const defaultModel = hint
                  ...
                  : null;
                // mooncode's `models.list` can repeat the same qualified id (e.g.
