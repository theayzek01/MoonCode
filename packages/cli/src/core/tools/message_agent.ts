...
const messageAgentSchema = Type.Object({
	targetAgentId: Type.String({ description: "The ID (TaskName) of the agent to send the message to. You can list active agents by passing 'list' as the targetAgentId." }),
	message: Type.Optional(Type.String({ description: "The message payload to send. Required if targetAgentId is not 'list'." })),
});
...
export function createMessageAgentToolDefinition(): ToolDefinition<typeof messageAgentSchema, undefined, any> {
	return {
		name: "message_agent",
		label: "message agent",
		description: "Multi-Agent Swarm: Send an IPC message to another active sub-agent. This allows collaboration between specialized agents (e.g. Researcher sending docs to Coder).",
		...
		async execute(_toolCallId, { targetAgentId, message }, _signal, _onUpdate, _ctx) {
			if (targetAgentId === "list") {
				...
				if (agents.length === 0) return { content: [{ type: "text", text: "No active sub-agents found." }], details: undefined };
				return {
			...
			if (!targetEngine) {
				return {
					content: [{ type: "text", text: `Error: Sub-agent '${targetAgentId}' not found. It may have already finished or the ID is incorrect.` }],
					details: undefined,
				};
			}
