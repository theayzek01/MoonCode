 ...
 * Connects to the MoonAgent Chrome Extension to extract live DOM, CSS, and console errors
...
export function createVisualBridgeToolDefinition(extensionPort: number = 3133) {
	return {
		name: "visual_bridge",
		label: "visual_bridge",
		description: "Connects to the MoonAgent browser extension to read live CSS, DOM, or errors from the active tab.",
		...
		async execute(_id: string, input: any, signal?: AbortSignal) {
			...
			return new Promise<any>((resolve, reject) => {
				...
				req.on("error", (err) => {
					reject(
						new Error(
							`Could not connect to MoonAgent browser extension on port ${extensionPort}. Is it installed and running? Error: ${err.message}`,
						),
					);
				});
