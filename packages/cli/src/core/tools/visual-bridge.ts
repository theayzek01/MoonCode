...
const visualBridgeSchema = Type.Object({
    action: Type.Union(
        [
            Type.Literal("extract_css"),
            Type.Literal("extract_dom"),
            Type.Literal("get_console_errors")
        ],
        { description: "The visual bridge action to perform via the Chrome extension" }
    ),
    selector: Type.Optional(Type.String({ description: "CSS selector for the element to target (for CSS/DOM extraction)" }))
});

/**
 * Browser Bridge (Visual & Live Copying)
 * Connects to the Mooncli Chrome Extension to extract live DOM, CSS, and console errors
...
export function createVisualBridgeToolDefinition(extensionPort: number = 3133) {
    return {
        name: "visual_bridge",
        label: "visual_bridge",
        description: "Connects to the Mooncli browser extension to read live CSS, DOM, or errors from the active tab.",
        ...
        async execute(_id: string, input: any, signal?: AbortSignal) {
            ...
            return new Promise<any>((resolve, reject) => {
                ...
                req.on('error', (err) => {
                    reject(new Error(`Could not connect to Mooncli browser extension on port ${extensionPort}. Is it installed and running? Error: ${err.message}`));
                });
