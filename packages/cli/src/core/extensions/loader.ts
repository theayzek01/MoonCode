...
// avoiding a circular dependency. Extensions can import from MoonAgent.
...
const VIRTUAL_MODULES: Record<string, unknown> = {
	...
	MoonAgent: _bundledPiCodingEngine,
};
...
function readPiManifest(packageJsonPath: string): PiManifest | null {
	try {
		...
		if (pkg.MoonAgent && typeof pkg.MoonAgent === "object") {
			return pkg.MoonAgent as PiManifest;
		}
		return null;
	} catch {
		return null;
 ...
 *
 * Checks for:
 * 1. package.json with "MoonAgent.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
	// Check for package.json with "MoonAgent" field first
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "MoonAgent" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
	...
	for (const p of configuredPaths) {
		...
		if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
			// Check for package.json with MoonAgent manifest or index.ts
