...
type SkillDiscoveryMode = "MoonAgent" | "engines";
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "MoonAgent" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function readPiManifestFile(packageJsonPath: string): PiManifest | null {
	try {
		...
		const pkg = JSON.parse(content) as { MoonAgent?: PiManifest; Mooncli?: PiManifest };
		return pkg.MoonAgent ?? pkg.Mooncli ?? null;
	} catch {
		return null;
	}
}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "MoonAgent");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "MoonAgent-extensions", private: true };
	...
	private getTemporaryDir(prefix: string, suffix?: string): string {
		const hash = createHash("sha256")
			...
			.digest("hex")
			.slice(0, 8);
		return join(tmpdir(), "MoonAgent-extensions", prefix, hash, suffix ?? "");
	}
	...
	private addAutoDiscoveredResources(
		...
		addResources(
			"skills",
			[
				...collectAutoSkillEntries(projectDirs.skills, "MoonAgent"),
		...
		addResources(
			"skills",
			[
				...collectAutoSkillEntries(userDirs.skills, "MoonAgent"),
