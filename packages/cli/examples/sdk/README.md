...
Programmatic usage of MoonCode-cli via `createEngineSession()` and `createEngineSessionRuntime()`.
...
```typescript
import { getModel } from "@MoonCode/core";
import {
  AuthStorage,
  createEngineSession,
...
// Minimal
const { session } = await createEngineSession({ authStorage, modelRegistry });

// Custom model
const model = getModel("anthropic", "claude-opus-4-5");
const { session } = await createEngineSession({ model, thinkingLevel: "high", authStorage, modelRegistry });
...
await loader.reload();
const { session } = await createEngineSession({ resourceLoader: loader, authStorage, modelRegistry });

// Read-only
const { session } = await createEngineSession({ tools: readOnlyTools, authStorage, modelRegistry });
...
const resourceLoader = new DefaultResourceLoader({
  ...
  enginesFilesOverride: () => ({ enginesFiles: [] }),
...
| `engineDir` | `~/.MoonCode/engine` | Config directory |
...
| `settingsManager` | `SettingsManager.create(cwd, engineDir)` | Settings overrides |
