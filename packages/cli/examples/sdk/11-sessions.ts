...
import { createEngineSession, SessionManager } from "MoonCode";

// In-memory (no persistence)
const { session: inMemory } = await createEngineSession({
	sessionManager: SessionManager.inMemory(),
});
console.log("In-memory session:", inMemory.sessionFile ?? "(none)");

// New persistent session
const { session: newSession } = await createEngineSession({
	sessionManager: SessionManager.create(process.cwd()),
});
console.log("New session file:", newSession.sessionFile);

// Continue most recent session (or create new if none)
const { session: continued, modelFallbackMessage } = await createEngineSession({
...
if (sessions.length > 0) {
	const { session: opened } = await createEngineSession({
