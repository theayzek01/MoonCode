 ...
 *
 * A lightweight TUI chat client that spawns the engine in RPC mode.
...
import { type Component, Container, Input, matchesKey, ProcessTerminal, SelectList, TUI } from "mooncli-tui";
...
// ============================================================================
// Loading indicator: "Engine: Working." -> ".." -> "..." -> "."
// ============================================================================
...
class LoadingIndicator implements Component {
	...
	render(_width: number): string[] {
		return [`${BLUE}${BOLD}Engine:${RESET} ${DIM}Working${".".repeat(this.dots)}${RESET}`];
	}
}
...
async function main() {
	...
	if (engine.exitCode !== null) {
		console.error(`Engine exited immediately. Stderr:\n${stderr}`);
		process.exit(1);
	}
	...
	// -- Slash commands (local, not sent to engine) --
	...
	// -- Process engine stdout --

	const stdoutRl = readline.createInterface({ input: engine.stdout!, terminal: false });
		...
		if (data.type === "message_update") {
			...
			if (evt?.type === "text_delta") {
				if (!hasTextOutput) {
					hasTextOutput = true;
					outputLog.append("");
					outputLog.append(`${BLUE}${BOLD}Engine:${RESET}`);
				}
