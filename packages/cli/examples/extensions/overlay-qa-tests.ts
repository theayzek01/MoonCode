 ...
 *
 * Usage: Hodeus --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "hodeus-tui";
import { matchesKey, truncateToWidth, visibleWidth } from "hodeus-tui";

// Global handle for toggle demo (in real code, use a more elegant pattern)
let globalToggleHandle: OverlayHandle | null = null;

export default function (Hodeus: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like Hodeus-doom would need)
	Hodeus.registerCommand("overlay-animation", {
	...
	Hodeus.registerCommand("overlay-anchors", {
	...
	Hodeus.registerCommand("overlay-margins", {
	...
	Hodeus.registerCommand("overlay-stack", {
	...
	Hodeus.registerCommand("overlay-overflow", {
	...
	Hodeus.registerCommand("overlay-edge", {
	...
	Hodeus.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	Hodeus.registerCommand("overlay-maxheight", {
	...
	Hodeus.registerCommand("overlay-sidepanel", {
	...
	Hodeus.registerCommand("overlay-toggle", {
	...
	Hodeus.registerCommand("overlay-passive", {
	...
	Hodeus.registerCommand("overlay-focus", {
	...
	Hodeus.registerCommand("overlay-streaming", {
...
class StreamingOverflowComponent extends BaseOverlay {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			echo "This simulates subengine output with colors, hyperlinks, and long paths"
			echo ""
			for i in $(seq 1 100); do
				# Simulate long file paths with OSC 8 hyperlinks (clickable) - tests width overflow
				DIR="/Users/nicobailon/Documents/development/Hodeus-mono/packages/cli/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/Hodeus-mono\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like Hodeus-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (Hodeus-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
