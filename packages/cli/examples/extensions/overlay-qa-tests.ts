 ...
 *
 * Usage: moodcli --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
export default function (moodcli: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like moodcli-doom would need)
	moodcli.registerCommand("overlay-animation", {
	...
	moodcli.registerCommand("overlay-anchors", {
	...
	moodcli.registerCommand("overlay-margins", {
	...
	moodcli.registerCommand("overlay-stack", {
	...
	moodcli.registerCommand("overlay-overflow", {
	...
	moodcli.registerCommand("overlay-edge", {
	...
	moodcli.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	moodcli.registerCommand("overlay-maxheight", {
	...
	moodcli.registerCommand("overlay-sidepanel", {
	...
	moodcli.registerCommand("overlay-toggle", {
	...
	moodcli.registerCommand("overlay-passive", {
	...
	moodcli.registerCommand("overlay-focus", {
	...
	moodcli.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/moodcli-mono/packages/cli/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/moodcli-mono\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like moodcli-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (moodcli-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
