 ...
 *
 * Demonstrates Hodeus.sendUserMessage() for sending user messages from extensions.
 * Unlike Hodeus.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (Hodeus: ExtensionAPI) {
	...
	Hodeus.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			Hodeus.sendUserMessage(args);
		},
	});

	// Command that steers the engine mid-conversation
	Hodeus.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				Hodeus.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	Hodeus.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				Hodeus.sendUserMessage(args, { deliverAs: "followUp" });
	...
	Hodeus.registerCommand("askwith", {
