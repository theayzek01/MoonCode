...
Example extensions for moodcli-cli.

## Usage

```bash
# Load an extension with --extension flag
moodcli --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.moodcli/engine/extensions/
```
...
| `structured-output.ts` | Final structured-output tool that returns `terminate: true` so the engine can end on the tool call |
| `built-in-tool-renderer.ts` | Custom compact rendering for built-in tools (read, bash, edit, write) while keeping original behavior |
| `minimal-mode.ts` | Override built-in tool rendering for minimal display (only tool calls, no output in collapsed mode) |
| `truncated-tool.ts` | Wraps ripgrep with proper output truncation (50KB/2000 lines) |
| `ssh.ts` | Delegate all tools to a remote machine via SSH using pluggable operations |
| `subengine/` | Delegate tasks to specialized subengines with isolated context windows |
...
| `send-user-message.ts` | Demonstrates `moodcli.sendUserMessage()` for sending user messages from extensions |
...
| `notify.ts` | Desktop notifications via OSC 777 when engine finishes (Ghostty, iTerm2, WezTerm) |
| `titlebar-spinner.ts` | Braille spinner animation in terminal title while the engine is working |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs moodcli theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `moodcli.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using moodcli-ai's built-in Anthropic/OpenCore streaming via proxy |
...
```typescript
import type { ExtensionAPI } from "@moodcli/cli";
