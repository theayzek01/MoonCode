 ...
 *
 * Usage:
 *   MoonCode -e ./ssh.ts --ssh user@host
 *   MoonCode -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (MoonCode: ExtensionAPI) {
	MoonCode.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	MoonCode.on("session_start", async (_event, ctx) => {
		...
		const arg = MoonCode.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
