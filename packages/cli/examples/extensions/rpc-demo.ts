...
export default function (moodcli: ExtensionAPI) {
	...
	moodcli.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "moodcli RPC Demo (new session)" : "moodcli RPC Demo");
	...
	moodcli.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	moodcli.on("turn_end", async (_event, ctx) => {
	...
	moodcli.registerCommand("rpc-input", {
	...
	moodcli.registerCommand("rpc-editor", {
	...
	moodcli.registerCommand("rpc-prefill", {
