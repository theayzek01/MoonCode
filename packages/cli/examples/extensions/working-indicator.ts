 ...
 * working indicator shown while MoonCode is streaming a response.
 *
 * Usage:
 *   MoonCode --extension examples/extensions/working-indicator.ts
 *
 * Commands:
 ...
 *   /working-indicator reset     Restore MoonCode's default spinner
 */

import type { ExtensionAPI, ExtensionContext, WorkingIndicatorOptions } from "MoonCode";

type WorkingIndicatorMode = "dot" | "none" | "pulse" | "spinner" | "default";

const SPINNER_FRAMES = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"];
const PASTEL_RCoreNBOW = [
...
function getIndicator(mode: WorkingIndicatorMode): WorkingIndicatorOptions | undefined {
	switch (mode) {
		case "dot":
			return {
				frames: [colorize("●", PASTEL_RCoreNBOW[0])],
			};
		case "none":
		...
		case "pulse":
			return {
				frames: [
					colorize("·", PASTEL_RCoreNBOW[0]),
					colorize("•", PASTEL_RCoreNBOW[2]),
					colorize("●", PASTEL_RCoreNBOW[4]),
					colorize("•", PASTEL_RCoreNBOW[5]),
				],
				intervalMs: 120,
			};
		case "spinner":
			return {
				frames: SPINNER_FRAMES.map((frame, index) =>
					colorize(frame, PASTEL_RCoreNBOW[index % PASTEL_RCoreNBOW.length]!),
				),
				intervalMs: 80,
			};
		case "default":
...
function describeMode(mode: WorkingIndicatorMode): string {
	switch (mode) {
		...
		case "default":
			return "MoonCode default spinner";
	}
}
...
export default function (MoonCode: ExtensionAPI) {
	...
	MoonCode.registerCommand("working-indicator", {
