 ...
 * (logo + keybinding hints) with a custom component showing the MoonCode mascot.
 */

import type { ExtensionAPI, Theme } from "MoonCode";
import { VERSION } from "MoonCode";

// --- PI MASCOT ---
// Based on pi_mascot.ts - the MoonCode engine character
...
export default function (MoonCode: ExtensionAPI) {
			...
			ctx.ui.setHeader((_tui, theme) => {
				return {
					render(_width: number): string[] {
						...
						const subtitle = `${theme.fg("muted", "   shitty coding engine")}${theme.fg("dim", ` v${VERSION}`)}`;
	...
	MoonCode.registerCommand("builtin-header", {
