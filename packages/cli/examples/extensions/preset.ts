 ...
 * - ~/.moodcli/engine/presets.json (global)
...
import { Container, Key, type SelectItem, SelectList, Text } from "@moodcli/tui";
import type { ExtensionAPI, ExtensionContext } from "moodcli";
import { DynamicBorder, getEngineDir } from "moodcli";
...
export default function presetExtension(moodcli: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify(
				"No presets defined. Add presets to ~/.moodcli/engine/presets.json or .moodcli/presets.json",
				"warning",
			);
			return;
		}
