 ...
 *
 * Usage:
 *   MoonCode -e examples/extensions/interactive-shell.ts
 *
 ...
 *
 * Note: This only intercepts user `!` commands, not engine bash tool calls.
...
export default function (MoonCode: ExtensionAPI) {
	MoonCode.on("user_bash", async (event, ctx) => {
