...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "MoonCode";

export default function (MoonCode: ExtensionAPI) {
	MoonCode.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
