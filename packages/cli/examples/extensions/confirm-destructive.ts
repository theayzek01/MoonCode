...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "Hodeus";

export default function (Hodeus: ExtensionAPI) {
	Hodeus.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
