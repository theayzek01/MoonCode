 ...
 * The engine plays via a single tool `tic_tac_toe` that takes ONE atomic action
 * per call. To play at (r, c) from its cursor (r0, c0) the engine must emit the
 * required move_* and a final `play` as SEPARATE tool_use blocks inside ONE
 * assistant response.
 *
 * Move actions share the engine cursor and have a 300ms delay. Under the
 ...
 *
 * The user cursor (TUI-only) and the engine cursor (tool-only) are stored in
 * separate variables. Only the engine cursor is ever exposed to the engine.
 */

import type { ExtensionAPI, ExtensionContext, Theme, ToolExecutionMode } from "MoonCode";
...
interface GameState {
	board: Cell[][];
	// User cursor (TUI-only, never exposed to the engine).
	userCursorRow: number;
	userCursorCol: number;
	// Engine cursor (manipulated by the tool, shown in the TUI during O's turn).
	engineCursorRow: number;
	engineCursorCol: number;
	status: GameStatus;
	userMark: Cell;
	engineMark: Cell;
	currentTurn: Cell;
...
// engine as `details`. Only the engine cursor is included: the user cursor is
// private to the TUI.
...
// Engine cursor home: where the cursor is reset to after a SUCCESSFUL play.
...
// nor on a failed `play` (cell taken), so the engine can retry without
// starting over.
...
function createInitialState(): GameState {
	return {
		...
		userCursorRow: 1,
		userCursorCol: 1,
		engineCursorRow: AGENT_CURSOR_HOME_ROW,
		engineCursorCol: AGENT_CURSOR_HOME_COL,
		status: "playing",
		userMark: "X",
		engineMark: "O",
		currentTurn: "X",
...
function boardToAscii(board: Cell[][], engineCursorRow: number, engineCursorCol: number): string {
	// Plain grid with coordinates for empty cells, marking the engine cursor
	// position with angle brackets. The user cursor is NEVER included: it is a
	// TUI-only concept and must not leak to the engine.
	const rows = board.map((row, r) =>
		row
			.map((c, cIdx) => {
				const onCursor = r === engineCursorRow && cIdx === engineCursorCol;
...
interface BoardRenderOpts {
	...
	cursor?: { row: number; col: number; owner: "user" | "engine" };
}
...
function renderBoard(opts: BoardRenderOpts): string[] {
	...
	// Green for user cursor, yellow for engine cursor.
	const cursorSgr = cursor?.owner === "engine" ? "\x1b[33;1m" : "\x1b[32;1m";
...
function renderVisualBoard(state: GameState, maxWidth: number): string[] {
	...
	const cursor =
		state.status !== "playing"
			? undefined
			: {
					row: isUserTurn ? state.userCursorRow : state.engineCursorRow,
					col: isUserTurn ? state.userCursorCol : state.engineCursorCol,
					owner: (isUserTurn ? "user" : "engine") as "user" | "engine",
				};
...
class TicTacToeComponent implements Component {
	...
	render(width: number): string[] {
		...
		if (this.state.status !== "playing") {
			...
			lines.push(centerPad(`Turn: ${bold(blue("X"))} (You)  ${dim("|")}  ${bold(yellow("O"))} (Engine)`, width));
		} else {
			lines.push(centerPad(`${blue("X")} (You)  ${dim("|")}  Turn: ${bold(yellow("O"))} (Engine)`, width));
		}
	...
	render(width: number): string[] {
		...
		switch (this.status) {
			case "win_X":
				...
				sub = "You beat the engine.";
				break;
			case "win_O":
				title = bold(this.theme.fg("warning", "\u2605 Player O wins \u2605"));
				sub = "The engine beat you.";
				break;
			case "draw":
...
function reconstructState(ctx: ExtensionContext): void {
	...
	for (const entry of ctx.sessionManager.getBranch()) {
		...
		if (details) {
			...
			gameState.engineCursorRow = details.engineCursorRow;
			gameState.engineCursorCol = details.engineCursorCol;
...
function getBoardDetails(): BoardDetails {
	return {
		...
		engineCursorRow: gameState.engineCursorRow,
		engineCursorCol: gameState.engineCursorCol,
...
export default function (MoonCode: ExtensionAPI) {
	MoonCode.on("session_start", async (_event, ctx) => reconstructState(ctx));
	MoonCode.on("session_tree", async (_event, ctx) => reconstructState(ctx));
	...
	// Provider (in case the message ends up in future context).
	const emitGameOverMessage = (): void => {
		const label =
			gameState.status === "win_X"
				? "Player X (human) wins"
				: gameState.status === "win_O"
					? "Player O (engine) wins"
	...
	// -----------------------------------------------------------------------
	MoonCode.registerMessageRenderer(MOVE_MESSAGE_TYPE, (message, { expanded }, theme) => {
		const details = message.details as BoardDetails | undefined;
		const turnLabel =
			details?.currentTurn === "O"
				? `${theme.fg("warning", theme.bold("O"))} (Engine)`
	...
	// -----------------------------------------------------------------------
	MoonCode.registerMessageRenderer(GAME_OVER_MESSAGE_TYPE, (message, _options, theme) => {
		const details = message.details as BoardDetails | undefined;
...
`;
	...
	// /tic-tac-toe command
	// -----------------------------------------------------------------------
	MoonCode.registerCommand("tic-tac-toe", {
		...
		handler: async (_args, ctx) => {
			...
			gameActive = true;
			MoonCode.setSessionName("Tic-Tac-Toe");
			...
			await ctx.ui.custom<void>((tui, _theme, _kb, done) => {
				component = new TicTacToeComponent(
					...
					(row, col) => {
						...
						MoonCode.appendEntry(SAVE_TYPE, getBoardDetails());

						if (gameState.status === "playing") {
							// IMPORTANT: user play does NOT touch the engine cursor.
							// The engine cursor is only reset after a successful engine play.
							...
							MoonCode.sendMessage(
								{
									...
									content:
										...
										`Your cursor is at (row=${gameState.engineCursorRow}, col=${gameState.engineCursorCol}). ` +
	...
	MoonCode.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
			...
			switch (params.action) {
				case "move_up":
					if (gameState.engineCursorRow > 0) gameState.engineCursorRow--;
					result = `Moved up. Cursor: (${gameState.engineCursorRow}, ${gameState.engineCursorCol})`;
					break;
				case "move_down":
					if (gameState.engineCursorRow < 2) gameState.engineCursorRow++;
					result = `Moved down. Cursor: (${gameState.engineCursorRow}, ${gameState.engineCursorCol})`;
					break;
				case "move_left":
					if (gameState.engineCursorCol > 0) gameState.engineCursorCol--;
					result = `Moved left. Cursor: (${gameState.engineCursorRow}, ${gameState.engineCursorCol})`;
					break;
				case "move_right":
					if (gameState.engineCursorCol < 2) gameState.engineCursorCol++;
					result = `Moved right. Cursor: (${gameState.engineCursorRow}, ${gameState.engineCursorCol})`;
					break;
				case "play": {
					if (gameState.status !== "playing") {
						throw new TicTacToeError(`Game is over (${gameState.status}).`);
					}
					if (gameState.currentTurn !== gameState.engineMark) {
						throw new TicTacToeError("It is not your turn.");
					}
					const r = gameState.engineCursorRow;
					const c = gameState.engineCursorCol;
					...
					gameState.board[r][c] = gameState.engineMark;
					gameState.status = checkWin(gameState.board);
					// Reset engine cursor to home ONLY on successful play.
					gameState.engineCursorRow = AGENT_CURSOR_HOME_ROW;
					gameState.engineCursorCol = AGENT_CURSOR_HOME_COL;
					if (gameState.status === "playing") {
	...
	MoonCode.registerTool({
		...
		async execute(_toolCallId, _params, _signal, _onUpdate, _ctx) {
			const boardAscii = boardToAscii(gameState.board, gameState.engineCursorRow, gameState.engineCursorCol);
			const text =
				`Board (your cursor marked with <>):\n${boardAscii}\n\n` +
				`Your cursor: (row=${gameState.engineCursorRow}, col=${gameState.engineCursorCol})\n` +
				`Status: ${gameState.status}\n` +
				`Turn: ${gameState.currentTurn === gameState.engineMark ? "Player O (you)" : "Player X"}`;
			return {
		...
		renderResult(result, { expanded }, theme) {
			...
			const summary =
				theme.fg("success", "\u2713 ") +
				theme.fg("muted", `cursor (${details?.engineCursorRow ?? 0},${details?.engineCursorCol ?? 0})`);
			if (expanded && details) {
				return new BannerMessageComponent(summary, details, true, theme);
			}
			return new Text(summary, 0, 0);
