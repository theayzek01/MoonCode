...
import { type Component, matchesKey, Text, truncateToWidth, visibleWidth } from "@moodcli/tui";
import type { ExtensionAPI, ExtensionContext, Theme, ToolExecutionMode } from "moodcli";
...
export default function (moodcli: ExtensionAPI) {
	...
	// -----------------------------------------------------------------------
	moodcli.registerMessageRenderer(MOVE_MESSAGE_TYPE, (message, { expanded }, theme) => {
	...
	// -----------------------------------------------------------------------
	moodcli.registerMessageRenderer(GAME_OVER_MESSAGE_TYPE, (message, _options, theme) => {
		const details = message.details as BoardDetails | undefined;
...
`;
	...
	// /tic-tac-toe command
	// -----------------------------------------------------------------------
	moodcli.registerCommand("tic-tac-toe", {
		...
		handler: async (_args, ctx) => {
			...
			gameActive = true;
			moodcli.setSessionName("Tic-Tac-Toe");
			...
			await ctx.ui.custom<void>((tui, _theme, _kb, done) => {
				component = new TicTacToeComponent(
					...
					(row, col) => {
						...
						moodcli.appendEntry(SAVE_TYPE, getBoardDetails());
