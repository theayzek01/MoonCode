 ...
 *
 * Custom tools MUST truncate their output to avoid overwhelming the Provider context.
 ...
 * 3. Inform the Provider where to find the complete output
	...
	moodcli.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			if (truncation.truncated) {
				...
				const tempDir = await mkdtemp(join(tmpdir(), "moodcli-rg-"));
				...
				// Add truncation notice - this helps the Provider understand the output is incomplete
