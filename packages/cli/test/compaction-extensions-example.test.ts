...
describe("Documentation example", () => {
	it("custom compaction example should type-check correctly", () => {
		...
		const exampleExtension = (Mooncli: ExtensionAPI) => {
			Mooncli.on("session_before_compact", async (event: SessionBeforeCompactEvent, ctx) => {
	...
	it("compact event should have correct fields", () => {
		const checkCompactEvent = (Mooncli: ExtensionAPI) => {
			Mooncli.on("session_compact", async (event: SessionCompactEvent) => {
