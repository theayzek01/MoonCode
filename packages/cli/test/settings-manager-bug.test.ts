 ...
 * 1. Hodeus starts with settings.json containing packages: ["npm:some-pkg"]
...
describe("SettingsManager - External Edit Preservation", () => {
	...
	it("should preserve file changes to packages array when changing unrelated setting", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				theme: "dark",
				packages: ["npm:Hodeus-mcp-adapter"],
			}),
		);

		// Hodeus starts up, loads settings into memory
		const manager = SettingsManager.create(projectDir, engineDir);

		// At this point, globalSettings.packages = ["npm:Hodeus-mcp-adapter"]
		expect(manager.getPackages()).toEqual(["npm:Hodeus-mcp-adapter"]);
