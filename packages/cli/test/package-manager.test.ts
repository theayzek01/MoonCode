...
Content`,
		...
		it("should auto-discover root markdown skills from .mooncode skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .mooncode", async () => {
			const extDir = join(tempDir, ".mooncode", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".mooncode"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".mooncode", "extensions"), directoryLinkType);
				symlinkSync(sharedSkillsDir, join(tempDir, ".mooncode", "skills"), directoryLinkType);
				symlinkSync(sharedPromptsDir, join(tempDir, ".mooncode", "prompts"), directoryLinkType);
				symlinkSync(sharedThemesDir, join(tempDir, ".mooncode", "themes"), directoryLinkType);
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".mooncode", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json Mooncli.extensions in extensions setting", async () => {
			// Create a package with Mooncli.extensions in package.json
			...
			// Should find the extensions declared in package.json Mooncli.extensions
	...
	describe(".engines/skills auto-discovery", () => {
		...
		it("should keep ~/.engines/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localEngineDir = join(tempDir, ".mooncode", "engine");
		...
		it("should dedupe user skill entries when ~/.mooncode/engine/skills is a symlink to ~/.engines/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .mooncode auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".mooncode\n");

			const skillDir = join(tempDir, ".mooncode", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with Mooncli manifest", async () => {
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".mooncode", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .mooncode settings base", () => {
			...
			const rel = relative(join(tempDir, ".mooncode"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in Mooncli manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json Mooncli.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".mooncode", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".mooncode", "npm")],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".mooncode", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".mooncode", "npm", "node_modules", "project-current");
			...
			expect(runCommandSpy).toHaveBeenNthCalledWith(
				2,
				"npm",
				["install", "project-old@latest", "project-missing@latest", "--prefix", join(tempDir, ".mooncode", "npm")],
				undefined,
			);
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".mooncode", "npm", "node_modules", "example");
