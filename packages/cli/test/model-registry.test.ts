...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "hodeus-core";
import { getApiProvider } from "hodeus-core";
import { getOAuthProvider } from "hodeus-core/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `Hodeus-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
