...
import { type AutocompleteProvider, CombinedAutocompleteProvider, Container } from "@moodcli/tui";
...
describe("InteractiveMode.showLoadedResources", () => {
	...
	function createExtensionFixtures(): ExtensionFixture[] {
		return [
			{
				path: "/tmp/project/.moodcli/extensions/answer.ts",
				sourceInfo: createSourceInfo("/tmp/project/.moodcli/extensions/answer.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.moodcli/extensions",
				}),
			},
			{
				path: "/tmp/project/.moodcli/extensions/local-index/index.ts",
				sourceInfo: createSourceInfo("/tmp/project/.moodcli/extensions/local-index/index.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.moodcli/extensions",
			...
			{
				path: "/tmp/project/.moodcli/git/github.com/HazAT/moodcli-interactive-subengines/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.moodcli/git/github.com/HazAT/moodcli-interactive-subengines/extensions/index.ts",
					{
						source: "git:github.com/HazAT/moodcli-interactive-subengines",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.moodcli/git/github.com/HazAT/moodcli-interactive-subengines",
					},
				),
			},
			{
				path: "/tmp/project/.moodcli/git/github.com/HazAT/moodcli-interactive-subengines/extensions/subengines/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.moodcli/git/github.com/HazAT/moodcli-interactive-subengines/extensions/subengines/index.ts",
					{
						source: "git:github.com/HazAT/moodcli-interactive-subengines",
						scope: "project",
						origin: "package",
...
"[Extensions]
  @scope/moodcli-scoped, answer.ts, cli-extension.ts, HazAT/moodcli-interactive-subengines, HazAT/moodcli-interactive-subengines:subengines, local-index, moodcli-markdown-preview, user-index"`);
	});
...
"[Extensions]
  project
    /tmp/project/.moodcli/extensions/answer.ts
    /tmp/project/.moodcli/extensions/local-index
    git:github.com/HazAT/moodcli-interactive-subengines
      extensions
      extensions/subengines
    npm:@scope/moodcli-scoped
      extensions
    npm:moodcli-markdown-preview
      extensions
  user
  ...
  path
	...
	test("shows context paths relative to cwd while preserving full external paths", () => {
		const home = homedir();
		const cwd = path.join(home, "Development", "moodcli-mono");
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			cwd,
			contextFiles: [
				{ path: path.join(home, ".moodcli", "engine", "AGENTS.md") },
		...
		expect(output).toContain("~/.moodcli/engine/AGENTS.md, AGENTS.md");
	...
	test("shows full context paths when expanded", () => {
		...
		const output = renderAll(fakeThis.chatContainer).replace(/\\/g, "/");
		expect(output).toContain("[Context]");
		expect(output).toContain("~/.moodcli/engine/AGENTS.md");
		expect(output).toContain("~/Development/moodcli-mono/AGENTS.md");
		expect(output).not.toContain("~/.moodcli/engine/AGENTS.md, AGENTS.md");
	});
