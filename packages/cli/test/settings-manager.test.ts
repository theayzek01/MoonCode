...
describe("SettingsManager", () => {
	...
	describe("preserves externally added settings", () => {
		it("should preserve enabledModels when changing thinking level", async () => {
			...
			// Create SettingsManager (simulates moodcli starting up)
	...
	describe("project settings directory creation", () => {
		it("should not create .moodcli folder when only reading project settings", () => {
			// Create engine dir with global settings, but NO .moodcli folder in project
			const settingsPath = join(engineDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the .moodcli folder that beforeEach created
			rmSync(join(projectDir, ".moodcli"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, engineDir);

			// .moodcli folder should NOT have been created just from reading
			expect(existsSync(join(projectDir, ".moodcli"))).toBe(false);

			// Settings should still be loaded from global
			expect(manager.getTheme()).toBe("dark");
		});

		it("should create .moodcli folder when writing project settings", async () => {
			// Create engine dir with global settings, but NO .moodcli folder in project
			const settingsPath = join(engineDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// .moodcli folder should NOT exist yet
			expect(existsSync(join(projectDir, ".moodcli"))).toBe(false);
			...
			// Now .moodcli folder should exist
			expect(existsSync(join(projectDir, ".moodcli"))).toBe(true);

			// And settings file should be created
			expect(existsSync(join(projectDir, ".moodcli", "settings.json"))).toBe(true);
		});
	});
	...
	describe("getSessionDir", () => {
		...
		it("should return project sessionDir, overriding global", () => {
			...
			writeFileSync(join(projectDir, ".moodcli", "settings.json"), JSON.stringify({ sessionDir: "./sessions" }));
			const manager = SettingsManager.create(projectDir, engineDir);
