...
function parseArgs(argv: string[]): Args {
	...
	let sessionPath = resolve(join(tmpdir(), `Hodeus-sdk-codex-cache-probe-tool-loop-${Date.now()}.jsonl`));
...
function buildPrompt(turn: number): string {
	...
	const lines: string[] = [];
	lines.push("This is an automated prompt-cache tool-loop test executed through the Hodeus cli SDK.");
	...
	for (let i = 1; i <= 180; i++) {
		lines.push(
			`Turn ${turn} synthetic record ${String(i).padStart(3, "0")}: alpha beta gamma delta epsilon zeta eta theta iota kappa lambda mu nu xi omicron Hodeus rho sigma tau upsilon phi chi psi omega.`,
		);
	}
	lines.push("");
