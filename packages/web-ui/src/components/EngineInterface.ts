import { streamSimple, type ToolResultMessage, type Usage } from "@moodcli/core";
...
import type { Engine, EngineEvent } from "@moodcli/engine";
...
@customElement("engine-interface")
export class EngineInterface extends LitElement {
	// Optional external session: when provided, this component becomes a view over the session
	@property({ attribute: false }) session?: Engine;
	...
	private setupSessionSubscription() {
		...
		this._unsubscribeSession = this.session.subscribe(async (ev: EngineEvent) => {
			switch (ev.type) {
				case "message_start":
				case "turn_start":
				case "turn_end":
				...
				case "message_end":
					...
					break;
				case "engine_end":
					// Clear streaming container when engine finishes
					if (this._streamingContainer) {
	...
	public async sendMessage(input: string, attachments?: Attachment[]) {
		...
		if (!session) throw new Error("No session set on EngineInterface");
		if (!session.state.model) throw new Error("No model set on EngineInterface");
...
if (!customElements.get("engine-interface")) {
	customElements.define("engine-interface", EngineInterface);
}
