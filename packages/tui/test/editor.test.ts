...
function cleanLine(line: string): string {
	...
	// Remove leading box border "│ " and trailing " │"
	return stripped
		.replace(/^[│╭╰]\s?/, "")
		.replace(/\s?[│╮╯]$/, "")
		.trim();
...
function cleanBorderKeepVT(line: string): string {
	// Remove leading border and its styles
	let content = line.replace(/^(?:\x1b\[[0-9;]*[a-zA-Z])?\s*[│╭╰]\s*(?:\x1b\[[0-9;]*[a-zA-Z])?\s*/, "");
	// Remove trailing border and its styles
	content = content.replace(/\s*(?:\x1b\[[0-9;]*[a-zA-Z])?\s*[│╮╯]\s*(?:\x1b\[[0-9;]*[a-zA-Z])?\s*$/, "");
	return content;
}
...
describe("Editor component", () => {
	...
	describe("Grapheme-aware text wrapping", () => {
		...
		it("wraps long text with emojis at correct positions", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const width = 15; // contentWidth = 11, layoutWidth = 10 (fits 5 emojis)
		...
		it("wraps CJK characters correctly (each is 2 columns wide)", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const width = 15; // contentWidth = 11, layoutWidth = 10 (fits 5 CJK chars)
			...
			const contentLines = lines.slice(1, -1).map((l) => cleanLine(l));
		...
		it("handles mixed ASCII and wide characters in wrapping", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const width = 20; // contentWidth = 16, layoutWidth = 15
		...
		it("shows cursor at end of line before wrap, wraps on next char", () => {
			const baseWidth = 14; // contentWidth = 10, layoutWidth = 9
			for (const paddingX of [0, 1]) {
				const editor = new Editor(createTestTUI(baseWidth + 2 * paddingX), defaultEditorTheme, { paddingX });

				// Type 9 chars → fills layoutWidth exactly, cursor at end on same line
				for (const ch of "aaaaaaaaa") editor.handleInput(ch);
				let lines = editor.render(baseWidth + 2 * paddingX);
				...
				editor.handleInput("a");
				lines = editor.render(baseWidth + 2 * paddingX);
	...
	describe("Word wrapping", () => {
		...
		it("does not start lines with leading whitespace after word wrap", () => {
			...
			for (let i = 0; i < contentLines.length; i++) {
				const line = cleanLine(contentLines[i]!);
		...
		it("preserves multiple spaces within words on same line", () => {
			...
			const contentLine = cleanLine(lines[1]!);
		...
		it("handles single word that fits exactly", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const width = 15; // contentWidth = 11, layoutWidth = 10 (fits 10 chars exactly)
	...
	describe("Sticky column", () => {
		...
		it("handles editor resizes when preferredVisualCol is on the same line", () => {
			...
			// Set render width to 16, which maps to layoutWidth 11 (compensating for the 4 border cols and -1).
			editor.render(16);
		...
		it("handles editor resizes when preferredVisualCol is on a different line", () => {
			...
			// Narrow the editor.
			// Render with width 14 to give layoutWidth 9 (compensating for the 4 border cols and -1).
			editor.render(14);
		...
		it("rewrapped lines: target fits current visual column", () => {
			...
			// Narrow to width 14 (layoutWidth = 9).
		...
		it("rewrapped lines: target shorter than current visual column", () => {
			...
			// Narrow to width 14 (layoutWidth = 9). Moving down clamps to col 8
			editor.render(14);
			editor.handleInput("\x1b[B");
			assert.deepStrictEqual(editor.getCursor(), { line: 1, col: 8 });
	...
	describe("Paste marker atomic behavior", () => {
		...
		it("does not get stuck moving down from a multi-visual-line paste marker", () => {
			...
			// terminal (24). Word-wrap splits at the space before "lines",
			// producing:
