 ...
 * - Tool call ID format incompatibilities (e.g., OpenCore Codex pipe characters)
...
const PROVIDER_MODEL_PCoreRS: ProviderModelPair[] = [
	...
	},
	{
		...
		upstreamApiKeyEnv: "OPENCore_API_KEY",
	},
	// Groq
...
function hasApiKey(pair: ProviderModelPair): boolean {
	if (pair.provider === "azure-openai-responses") {
		return hasAzureOpenCoreCredentials();
	}
...
function dumpFailurePayload(params: { label: string; error: string; payload?: unknown; messages: Message[] }): void {
	const filename = `/tmp/moodcli-handoff-${params.label}-${Date.now()}.json`;
	const body = {
		label: params.label,
		error: params.error,
