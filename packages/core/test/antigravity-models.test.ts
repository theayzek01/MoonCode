...
describe("Antigravity model metadata", () => {
	test("matches ag-local-bridge public model ids and limits", () => {
		...
		expect(antigravityModels["antigravity-gemini-3-5-flash-medium"].maxTokens).toBe(65536);
		expect(antigravityModels["antigravity-gpt-oss-120b-medium"].contextWindow).toBe(128000);
		expect(antigravityModels["antigravity-gpt-oss-120b-medium"].maxTokens).toBe(16384);
	});

	test("maps prefixed public ids to Cloud Code Assist model ids", () => {
		const request = buildRequest(
			antigravityModels["antigravity-claude-sonnet-4-6-thinking"],
	...
	test("adds Claude thinking beta eligibility for antigravity provider ids", () => {
		expect(needsClaudeThinkingBetaHeader(antigravityModels["antigravity-claude-sonnet-4-6-thinking"])).toBe(true);
		expect(needsClaudeThinkingBetaHeader(antigravityModels["antigravity-gemini-3-5-flash-medium"])).toBe(false);
	});
});
