...
Moodcli saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.moodcli/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.

```bash
moodcli -c                  # Continue most recent session
moodcli -r                  # Browse and select from past sessions
moodcli --no-session        # Ephemeral mode; do not save
moodcli --session <path|id> # Use a specific session file or partial session ID
moodcli --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `moodcli -r` opens the same picker at startup.
...
When available, moodcli uses the `trash` CLI for deletion instead of permanently removing files.
...
Named sessions are easier to find in `/resume` and `moodcli -r`.
...
When `/tree` switches away from one branch to another, moodcli can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
