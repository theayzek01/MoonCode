> moodcli can create themes. Ask it to build one for your setup.
...
Moodcli loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.moodcli/agent/themes/*.json`
- Project: `.moodcli/themes/*.json`
- Packages: `themes/` directories or `moodcli.themes` entries in `package.json`
...
On first run, moodcli detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.moodcli/agent/themes
vim ~/.moodcli/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/badlogic/moodcli-mono/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, moodcli reloads it automatically for immediate visual feedback.
...
Moodcli uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, moodcli falls back to the nearest approximation.
