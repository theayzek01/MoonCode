> moodcli can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to moodcli's agent capabilities. Use it to embed moodcli in other applications, build custom interfaces, or integrate with automated workflows.
...
```typescript
import { AuthStorage, createAgentSession, ModelRegistry, SessionManager } from "@moodcli/coding-agent";
...
```bash
npm install @moodcli/coding-agent
```
...
```typescript
import { createAgentSession } from "@moodcli/coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `moodcli.sendMessage()`.
...
The `Agent` class (from `@moodcli/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.moodcli/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.moodcli/extensions/`)
- Project skills:
  - `.moodcli/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.moodcli/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.moodcli/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@moodcli/coding-agent";
...
// Default: uses ~/.moodcli/agent/auth.json and ~/.moodcli/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@moodcli/coding-agent";
...
- If you omit `tools`, moodcli automatically creates them with the correct `cwd`
...
import { createAgentSession, defineTool } from "@moodcli/coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `moodcli.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `moodcli.registerTool()`.

> See [examples/sdk/05-tools.ts](../examples/sdk/05-tools.ts)

### Extensions

Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.moodcli/agent/extensions/`, `.moodcli/extensions/`, and settings.json extension sources.
...
**Event Bus:** Extensions can communicate via `moodcli.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@moodcli/coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@moodcli/coding-agent";
...
1. Global: `~/.moodcli/agent/settings.json`
2. Project: `<cwd>/.moodcli/settings.json`
