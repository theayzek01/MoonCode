...
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "@moodcli/ai";
import { registerFauxProvider } from "@moodcli/ai";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `moodcli-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
