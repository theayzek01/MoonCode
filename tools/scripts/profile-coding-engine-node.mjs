...
function printHelp() {
	console.log(`Usage:
  node scripts/profile-cli-node.mjs [options]

Profiles cli startup with the runtime selected below:
- npm run profile:tui     -> builds packages/cli and profiles TUI startup with Node
- npm run profile:rpc     -> builds packages/cli and profiles RPC startup with Node
...
async function runBuild() {
	process.stdout.write("Building packages/tui, packages/ai, packages/agent, and packages/cli...\n");
...
async function main() {
	...
	if (runtime === "bun") {
		process.stdout.write(
			`Using Bun runtime with ${options.mode === "rpc" ? "packages/cli/src/cli.ts --mode rpc" : "packages/cli/src/cli.ts"}\n`,
		);
	}
